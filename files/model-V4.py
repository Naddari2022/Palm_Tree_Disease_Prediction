...
top_model_weights_path = 'vgg16_model.h5'
epochs = 10
batch_size = 16
#data directories
##################################extract file 
...
def save_bottleneck_features():   
    ...
    ###define image_data_genrator
    datagen  = ImageDataGenerator(
                    rescale=1./255,
                    validation_split=0.2
                    )
  ### configure it for training set
...
#     #EXTRACT BOTTLE NECK FEATURE FOR VALIDATION_DATA
    ...
    print("printing class indices ",generator.class_indices)
    print("printin numb of classes for validation:",num_classes)    
    print("length of the generator for validation:",len(generator.filenames))
    ...
    np.save(open('model3_feat_val.npy', 'wb'),
    ...
    class_weights_val = class_weight.compute_class_weight(
               'balanced',
                np.unique(generator.classes), 
                generator.classes)
    print("here new way of compute class weight_val:",class_weights_val)
    ...
    np.save(open('model3_feat_test.npy', 'wb'),
...
#########################################################  
def train_top_model():  
    # In order to train the top model, we need the class labels
    #for each of the training/validation samples.
   ...
   # np.save('model3_classIndices.npy', generator_top_train.class_indices)
    ...
    #print("here new way of compute class weight_train:",class_weights_train)
    train_data = np.load('model3_feat_train.npy')
    validation_data = np.load('model3_feat_val.npy')
    test_data = np.load('model3_feat_test.npy')
   ...
   #########################################
    #  or get the class lebels for the training data, in the original order  
   ...
   ###################################   
    ...
    #here compile the model with slow learning rate
...
new_model = tf.keras.models.load_model('model3.model')
test_data = np.load('model3_feat_test.npy')
